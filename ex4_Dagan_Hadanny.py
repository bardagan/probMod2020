...
class CollectSum:
    ...
    def collect_distribute_sum(self, root):
        ...
        return self.marginal_tables, self.collect_msgs, likelihood
...
# Calculates the log likelihood of data given a set of parameters.
def get_log_like(data, parameters):
    sum_log_likelihood = 0
    for edge, p_change in zip(edges, parameters):
        change = np.abs(data[:,edge[0]] - data[:,edge[1]])
        change_count = np.sum(change) # count of changes out of all observations for edge
        ...
        sum_log_likelihood += log_likelihood #sums over all observations
    return sum_log_likelihood + np.log(0.5) * data.shape[0]

# Calculate parameters from complete data
...
# uses complete data to infere parameters and calculate the log likelihood.
def inference_from_complete_data(filename):
    data= get_table(filename, [], list(range(1, 11))) #get data from file. Assumes complete data
    parameters = get_parameters(data) # calculate the parameters
...
def maximum_probability_inference(filename, initial_setting, observed_indices):
    ...
    history = []
    prev_likelihood = 0
    for i in range(100): #100 is max if doesn't converge
        ...
        if np.abs(prev_likelihood - new_likelihood) < 0.001:
            break
...
def expectation_maximization_inference(filename, initial_setting, observed_indices):
    ...
    for i in range(100): #100 is max if doesn't converge
        ...
        # Check for convergence (change in log likelihood)
        if np.abs(prev_likelihood - sum_log_likelihood) < 0.001:
            break
        prev_likelihood = sum_log_likelihood
...
if __name__ == "__main__":
    try:
        ...
        print("Usage ex4.py <option>  <filename> -  options are 'C', 'M', 'E'")
        exit()

    # Finds the best parameters for maximum inference by trying different random parameters
    if option == 'find_best_parameters_M':
        ...
        best_prob = None
        for _ in range(1000):
            initial_params = [uniform(0, 1) for _ in range(9)]
            probs = maximum_probability_inference(filename, initial_params, [3, 4, 6, 7, 9, 10])
            
            if best_prob is None or probs[-1][9] > best_prob:
                best_prob = probs[-1][9]
                best_start_params = initial_params
                final_params.append(probs[-1])
                best_probs = probs
        print(tabulate(best_probs, headers=headers))
        exit()

    # Finds the best parameters for EM by trying different random parameters
    if option == 'find_best_parameters_E':
        from random import uniform
        final_params = []
        headers = edges + ['log-prob', 'log-ld']
        best_start_params = None
        ...
        for i in range(100):
            if i % 20 == 0: print(i)
            initial_params = [uniform(0, 1) for _ in range(9)]
            probs = expectation_maximization_inference(filename, initial_params, [3, 4, 6, 7, 9, 10])
        ...
        print("\n")
        print(tabulate(best_probs, headers=headers))
        best_probs = list(np.round(best_probs, 3))
        shortened = best_probs[0:2]
        shortened.append(['...'] * 11)
        shortened.append(best_probs[-1])
        print("\n")
        print(tabulate(shortened, headers=headers))
        exit()

    if option =='C':
        print("\nInfering from complete data using data from file %s" % (filename))
        probabilities, likelihood = inference_from_complete_data(filename)
    ...
    try:
        initial_setting = []
        for i in range(3, 12):
            initial_setting.append(float(sys.argv[i]))
    except IndexError:
        print("For options 'M' or 'E' also input 9 initial settings for parameters")
        exit()

    if option == 'M':
        probs = maximum_probability_inference(filename, initial_setting, [3, 4, 6, 7, 9, 10])
        headers = edges + ['log-prob', 'log-ld']
        print(tabulate(probs, headers=headers))

    if option == 'E':
        probs = expectation_maximization_inference(filename, initial_setting, [3, 4, 6, 7, 9, 10])
        headers = edges + ['log-prob', 'log-ld']
        print(tabulate(probs, headers=headers))